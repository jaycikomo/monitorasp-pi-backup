...
app.get('/api/network-minimal', async (req, res) => {
    try {
        ...
        const devices = [
            ...
            { name: 'Répéteur Free', ip: '192.168.1.178', type: 'repeater' }
        ];
...
// Route pour la page d'analyse réseau avancée
app.get("/network-advanced", (req, res) => {
    res.render("network-advanced");
});
    res.send(`
<!DOCTYPE html>
<html lang="fr">
...
app.get('/api/new-cameras-status', async (req, res) => {
    ...
    const newCameras = [
        ...
        { id: 'cam12', name: 'CamIPC', ip: '10.0.1.41', interface: 'wlan1', emoji: '🎥' }
    ];
...
// API UPTIME KUMA - HEARTBEATS RÉELS
app.get('/api/uptime-kuma', async (req, res) => {
    try {
        const dbPath = '/var/lib/docker/volumes/46be38a5187c0075b16e6c5f4c649b991bc7c9e8687dea77924c0b2bc095ea41/_data/kuma.db';
        
        exec(`sudo sqlite3 "${dbPath}" "SELECT h.monitor_id, h.status, h.time, m.name FROM heartbeat h JOIN monitor m ON h.monitor_id = m.id WHERE h.time = (SELECT MAX(time) FROM heartbeat h2 WHERE h2.monitor_id = h.monitor_id) ORDER BY m.name;"`, (error, stdout) => {
            let heartbeatList = {};
            let uptimeList = {};
            ...
            if (!error && stdout) {
                ...
                lines.forEach(line => {
                    const [monitorId, status, time, name] = line.split('|');
                    if (monitorId && name) {
                        ...
                        uptimeList[monitorId] = parseInt(status) === 1 ? '99%+' : '0%';

                        if (parseInt(status) === 1) onlineCount++;
                        else offlineCount++;
                    }
                });
            }
...
// API pour ping d'une caméra spécifique
app.get('/api/ping-camera/:ip', async (req, res) => {
    const { exec } = require('child_process');
...
// API pour ping de toutes les caméras
app.get('/api/ping-all-cameras', async (req, res) => {
    const { exec } = require('child_process');
    
    // Liste des IPs des caméras à partir du fichier
    const cameras = [
        { id: 'cam1', ip: '10.0.1.161' },
        { id: 'cam2', ip: '10.0.1.162' },
        { id: 'cam3', ip: '10.0.1.169' },
        { id: 'cam4', ip: '10.0.1.163' },
        { id: 'cam5', ip: '10.0.1.164' },
        { id: 'cam6', ip: '10.0.1.165' },
        { id: 'cam7', ip: '10.0.1.166' },
        { id: 'cam8', ip: '10.0.1.167' },
        { id: 'cam9', ip: '10.0.1.168' },
        { id: 'cam10', ip: '10.0.1.200' },
        { id: 'cam11', ip: '192.168.1.44' },
        { id: 'cam12', ip: '10.0.1.41' }
    ];
    ...
    for (const camera of cameras) {
        try {
            const pingCmd = `ping -c 1 -W 3 ${camera.ip}`;
            const result = await new Promise((resolve) => {
                exec(pingCmd, (error, stdout) => {
                    if (error) {
                        ...
                        const timeMatch = stdout.match(/time=([0-9.]+) ms/);
                        const time = timeMatch ? parseFloat(timeMatch[1]) : 0;
                        resolve({ online: true, time: Math.round(time) });
                    }
                });
            });
            results[camera.id] = result;
        } catch (error) {
            results[camera.id] = { online: false };
        }
    }
...
// API pour ping d'un équipement spécifique
